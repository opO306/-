...
import { titleUnlockCost, titleUpgradeCost } from "../app/IncrementalGameApp"; // titleUnlockCost, titleUpgradeCost 임포트
import { TitleNode } from "../data/titles";
import { motion, AnimatePresence } from "framer-motion";
import { Button } from "@/app/components/ui/button";
import Decimal from "break_infinity.js";

interface TitleDialogProps {
  node: TitleNode | null; // 선택 노드 (null = 닫힘)
  onClose: () => void;
}
...
export default function TitleDialog({ node, onClose }: TitleDialogProps) {
  const [{ fame, titles }, dispatch] = useGame(); // titles 상태 가져오기
  const currentTitleLevel = node ? titles[node.id] || -1 : -1; // 현재 칭호 레벨 가져오기

  const unlockCostValue = node ? titleUnlockCost(node.tier) : new Decimal(0);
  const upgradeCostValue = node ? titleUpgradeCost(currentTitleLevel + 1) : new Decimal(0); // 다음 레벨 비용
  ...
  function handleUnlock(n: TitleNode) {
    dispatch({ type: "UNLOCK_TITLE", id: n.id });
    onClose();
  }

  function handleUpgrade(n: TitleNode) {
    dispatch({ type: "UPGRADE_TITLE", id: n.id });
  }
}
