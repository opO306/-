...
export const completeChallenge = functions.https.onCall(
  async (data: { uid: string; challengeId: string; completionTime: number; dailyChallenge: DailyChallengeInstance }, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError("unauthenticated", "The function must be called while authenticated.");
    }

    const { uid, challengeId, completionTime, dailyChallenge } = data;
    ...
    try {
      ...
      throw new functions.https.HttpsError("internal", "Failed to complete challenge.");
    }
  }
);
