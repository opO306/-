 ...
 */
export const genComposite = onCall(async (request: CallableRequest<{ job: Job; title: Title; archetype: ArchetypeVector }>) => {
  ...
  // 1. Rule Engine: (job.tags, title.tags) ↦ 프롬프트 스켈레톤 선택
  let promptSkeleton = `직업 '${job.name}' (태그: ${job.tags.join(', ')})과 칭호 '${title.label}' (태그: ${title.tags.join(', ')})의 조합에 어울리는 새로운 파생 직업의 이름과 보너스를 3가지 제안해주세요. 플레이어의 성향은 ${JSON.stringify(archetype)}입니다.`;
